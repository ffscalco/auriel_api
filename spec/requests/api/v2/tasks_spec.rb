...
RSpec.describe 'Tasks API', type: :request do
  ...
  let(:headers) do
    {
      'Accept' => 'application/vnd.aurielapi.v2',
  ...
  describe 'GET /tasks' do
    context 'when no filter param is set' do
      before do
    ...
    context 'when filter and sorting  params are sent' do
      ...
      before do
        get '/tasks', params: {q: {title_cont: :note, s: :'title+ASC'}}, headers: headers
      end

      it 'returns only the tasks matching' do
        returned_task_titles = json_body[:data].map { |t| t[:attributes][:title] }

        expect(returned_task_titles).to match_array([notebook_task_1.title, notebook_task_2.title])
      end
    end
  end
